...
TouchTracker touchTracker = new TouchTracker(10); 
...
void DrawTouches() {
  for (int i = 0; i < touchTracker.touchCount(); i++) {
    if (touchTracker.GetTouch(i).isActive()) {
...
public void drawBlobs() {
  ...
  for (int n = 0; n < blobDetection.getBlobNb(); n++) {
    ...
    if (blob != null) {
      ...
      touchTracker.UpdateTouchPosition(pos, floor((blob.w* destImg.width + blob.h* destImg.height)  / 2));
    }
  }
